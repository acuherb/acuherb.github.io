...
document.addEventListener('DOMContentLoaded', function() {
  // 监听 Twikoo 评论框的动态加载（适用于异步加载场景）
  const observer = new MutationObserver(function() {
    const twikooInput = document.querySelector('.tk-input');
    if (twikooInput && !twikooInput.querySelector('.tk-comment-animated-bg')) {
      // 创建动态背景元素
      const bg = document.createElement('div');
      bg.className = 'tk-comment-animated-bg';
